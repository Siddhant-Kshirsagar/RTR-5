...
void display(void)
{
	...
	float length = 0.0f;
	//function declaration
	//1st param length of side
	//2nd param struct MYCOLOR for color 
	void drawTriangle(float,struct MYCOLOR);
	//code
	...
	//triangle 
	for (length = 1.1f,i = 0; length >= 0.1f; length = length - 0.1f, i++)
	{
		drawTriangle(length, myColor[i]);
	}
...
void drawTriangle(float length,struct MYCOLOR color)
{
