...
struct MYCOLOR
{
	float rColor; // red color 
	float gColor; // green color 
	float bColor; // blue color
}myColor[10] = {  { 1.0f,0.0f,0.0f } //red 
				...
				, { 1.0f,128.0f / 255.0f,0.0f } //orange
				, { 192.0f / 255.0f,192.0f / 255.0f,192.0f / 255.0f } //light gray
				, { 128.0f / 255.0f,0.0f,128.0f / 255.0f } //purple
				};
...
void display(void)
{
	...
	//function declaration
	//1st param radius
	//2nd param struct MYCOLOR for color 
	void drawCircle(float,struct MYCOLOR);
	//code
	...
	//triangle 
	for (radius = 1.1f,i = 0; radius >= 0.1f; radius = radius - 0.1f, i++)
	{
		drawCircle(radius, myColor[i]);
	}
...
void drawCircle(float radius,struct MYCOLOR color)
{
	//variable declaration
	float x, y;
	...
	for (float fAngle = 0.0f; fAngle <= 360.0f; fAngle = fAngle + 0.1f)
	{
		...
		y = 0.0f + radius * sin((fAngle * M_PI) / 180.0f);
		glVertex3f(x, y, 0.0f);
	}
	glEnd();
}
