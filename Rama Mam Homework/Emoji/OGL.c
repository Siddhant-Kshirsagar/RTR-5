...
float circleCordinate[] = { 0.0f,0.0f,0.0f }; // for circle coordinates
float yAxisLip = -0.15f; // for control lip expression 
float xAxisLipNegative = -0.1f; // for control lip expression 
float xAxisLipPositive = 0.1f; // for control lip expression 
BOOL bSmile = FALSE; // for smile/sad expression animation
...
{
	//function declaration
	void DrawCircle(float, float, float,struct RGBColor); // 1st parameter x cordinate  , 2nd parameter y cordinate , 3rd parameter radius of circle , 4th parameter RGB value for circle

	void DrawLip(void); 

	//variable declaration
	struct RGBColor myColor = { 0.0f, 0.0f,0.0f};
	...
	// yellow color for rgb
	myColor.fRed = 1.0f;
	myColor.fGreen = 1.0f;
	myColor.fBlue = 0.0f;

	DrawCircle(0.0f, 0.0f, 0.3f,myColor); // face (main circle)

	// black color for rgb
	myColor.fRed = 0.0f;
	myColor.fGreen = 0.0f;
	myColor.fBlue = 0.0f;
	DrawCircle(-0.15f, 0.1f, 0.05f, myColor); // left eye
	DrawCircle(0.15f, 0.1f, 0.05f, myColor); // right eye
...
void DrawCircle(float xAxis, float yAxis, float radius, struct RGBColor rgbColor)
{
	glBegin(GL_LINES);
	...
	{
		circleCordinate[0] = xAxis + radius * cos(angle);
		circleCordinate[1] = yAxis + radius * sin(angle);

		glColor3f(rgbColor.fRed, rgbColor.fGreen, rgbColor.fBlue);

		glVertex3f(xAxis, yAxis, 0.0f);
		glVertex3f(circleCordinate[0], circleCordinate[1], circleCordinate[2]);
	}
...
void DrawLip(void)
{
	...
	//left line of lip
	glVertex3f(-0.15, yAxisLip, 0.0f);
	glVertex3f(xAxisLipNegative, -0.15f, 0.0f);

	//mid line of lip
	glVertex3f(xAxisLipNegative, -0.15f, 0.0f);
	glVertex3f(xAxisLipPositive, -0.15f, 0.0f);

	//right line of lip
	glVertex3f(xAxisLipPositive, -0.15f, 0.0f);
	glVertex3f(0.15f, yAxisLip, 0.0f);
...
void update(void)
{
	...
	if (xAxisLipNegative < -0.5f)
	{
		xAxisLipNegative = xAxisLipNegative + 0.0001f;
	}

	if (xAxisLipPositive > 0.5f)
	{
		xAxisLipPositive = xAxisLipPositive - 0.0001f;
	}
}
