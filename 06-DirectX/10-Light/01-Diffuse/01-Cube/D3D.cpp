...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"}" \
		"float4 position = mul(perspectiveMatrix, mul(worldViewMatrix,pos));" \
	...
	const char *pixelShaderSourceCode =
		...
		"{"
		"float4 color = float4(input.diffuseLight,1.0);" \
		"return color;" \
		"}";
	...
	numOfElements = sizeof(cubePositions) / sizeof(cubePositions[0]);
	...
	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC; // similar to openGL GL_DYNAMIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(cubeNormals);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
...
void display(void)
{
	...
	if (bLightingEnable == TRUE)
	{
		...
		/*XMStoreFloat3(&constantBuffer.LD, XMVectorSet(0.0f, 1.0f, 0.0f, 1.0f));
		XMStoreFloat3(&constantBuffer.KD, XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f));*/
