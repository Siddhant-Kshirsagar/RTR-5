...
HRESULT initialize(void)
{
	...
	numOfElements = sizeof(pyramidPosition) / sizeof(pyramidPosition[0]);
	...
	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC; // similar to openGL GL_DYNAMIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(pyramidNormals);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	// create vertex buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_NormalBuffer);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for normal vertex buffer\n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() succeeded for normal vertex buffer\n\n");
		fclose(gpFILE);
	}
	...
	// now map our position buffer with above resource
	gpID3D11DeviceContext->Map(gpID3D11Buffer_NormalBuffer,
		0,
		D3D11_MAP_WRITE_DISCARD,
		0,
		&d3dMappedSubresource);
	...
	light[1].ambient[0] = 1.0f;
	light[1].ambient[1] = 1.0f;
	light[1].ambient[2] = 1.0f;
