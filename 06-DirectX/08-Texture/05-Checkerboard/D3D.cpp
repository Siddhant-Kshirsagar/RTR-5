...
HRESULT initialize(void)
{
	...
	const float rectangle_texcoord[] =
	{
		0.0f,1.0f, // LT
		1.0f,1.0f, // RT
		0.0f,0.0f, // LB
	...
	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC; // similar to openGL GL_DYNAMIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(rectangle_texcoord);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	// create vertex buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_TexcoordBuffer);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for vertex buffer\n\n");
	...
	// now copy actual data
	memcpy(d3dMappedSubresource.pData, rectangle_texcoord, sizeof(rectangle_texcoord));
...
HRESULT loadD3DTexture(void)
{
	...
	d3dTexture2DDesc.Width = (UINT)CHECKIMAGEWIDTH;
	d3dTexture2DDesc.Height = (UINT)CHECKIMAGEHEIGHT;
	...
	d3dTexture2DDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	d3dTexture2DDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	d3dTexture2DDesc.CPUAccessFlags = 0;
	d3dTexture2DDesc.MiscFlags = 0;

	ID3D11Texture2D *pID3D11Texture2D_Checkerboard = NULL;

	D3D11_SUBRESOURCE_DATA d3d11SubresourceData;
	ZeroMemory((void *)&d3d11SubresourceData, sizeof(D3D11_SUBRESOURCE_DATA));

	d3d11SubresourceData.pSysMem = checkImage;
	d3d11SubresourceData.SysMemPitch = CHECKIMAGEWIDTH * 4;

	// create wic texture 
	hr = gpID3D11Device->CreateTexture2D(&d3dTexture2DDesc, &d3d11SubresourceData, &pID3D11Texture2D_Checkerboard);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateTexture2D() succeeded\n\n");
		fclose(gpFILE);
	}
	...
	gpID3D11Device->CreateShaderResourceView(pID3D11Texture2D_Checkerboard, &d3d11ShaderResourceViewDesc, &gpID3D11ShaderResourceView);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateShaderResourceView() failed\n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateShaderResourceView() succeeded\n\n");
		fclose(gpFILE);
	}
...
void makeCheckImage(void)
{
	...
	for (i = 0; i < CHECKIMAGEHEIGHT; i++)
	{
		for (j = 0; j < CHECKIMAGEWIDTH; j++)
		{
			...
			checkImage[i][j][0] = (UINT)c;
			checkImage[i][j][1] = (UINT)c;
			checkImage[i][j][2] = (UINT)c;
			checkImage[i][j][3] = (UINT)c;
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationY(-(XMConvertToRadians(45.0)));
