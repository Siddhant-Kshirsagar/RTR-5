...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"};" \
		"vertex_output main(float4 pos:POSITION, float3 norm:NORMAL, float4 col:COLOR, float2 tex:TEXCOORD)" \
		"{" \
	...
	const char *pixelShaderSourceCode =
		...
		"}" \
		"float4 tex = myTexture2D.Sample(mySamplerState,input.texcoord);" \
		"float4 color = float4(phongADSLight * input.color * (float3)tex,1.0) ;" \
		"return color;" \
		"}";
	...
	d3d11BufferDesc.ByteWidth = sizeof(cube_PCNT);
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_PCNTBuffer);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_PCNTBuffer,
		0,
...
void display(void)
{
	...
	UINT stride = sizeof(float) * 11; // similar to glVertexAttribPointer() 3rd parameter
	...
	stride = sizeof(float) * 11; // similar to glVertexAttribPointer() 3rd parameter
	offSet = sizeof(float) * 3;// similar to glVertexAttribPointer() 6th parameter
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_PCNTBuffer, &stride, &offSet);

	//// set normal buffer into pipeline here
	stride = sizeof(float) * 11; // similar to glVertexAttribPointer() 3rd parameter
	offSet = sizeof(float) * 6;// similar to glVertexAttribPointer() 6th parameter
	...
	stride = sizeof(float) * 11; // similar to glVertexAttribPointer() 3rd parameter
	offSet = sizeof(float) * 9;// similar to glVertexAttribPointer() 6th parameter
