...
// constant buffer in direct X must be  multiple of and >= 16
...
unsigned int uiNumberOfLineSegment;
...
HRESULT initialize(void)
{
	...
	const char *hullShaderSourceCode =
		...
		"float numberOfStrip = hull_constant_function_params[0];" \
		"float numberOfSegment = hull_constant_function_params[1];" \
		"output.edges[0] = numberOfStrip;" \
		"output.edges[1] = numberOfSegment;" \
		"return(output);"
		"}" \
		"struct hull_output" \
		"{" \
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "Hull shader compile successfully\n\n");
		fclose(gpFILE);
	}


	// create hull shader from above code
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "Domain shader compile successfully\n\n");
		fclose(gpFILE);
	}


	// create domain shader from above code
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateDomainShader() failed\n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateDomainShader() succeded\n\n");
		fclose(gpFILE);
	}
	...
	numOfElements = sizeof(pointPosition) / sizeof(pointPosition[0]);
	...
	d3d11BufferDesc.ByteWidth = sizeof(pointPosition);
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for constant buffer hull shader\n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() succeeded for constant buffer hull shader \n\n");
		fclose(gpFILE);
	}
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for constant buffer DomainShader \n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() succeeded for constant buffer DomainShader\n\n");
		fclose(gpFILE);
	}
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for constant buffer PixelShader\n\n");
		fclose(gpFILE);
		return(hr);
	}
	else
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() succeeded for constant buffer PixelShader\n\n");
		fclose(gpFILE);
	}
...
void display(void)
{
	...
	gpID3D11DeviceContext->UpdateSubresource(
		gpID3D11Buffer_ConstantBuffer_DomainShader, 0, NULL,&constantBuffer_DomainShader, 0, 0);

	// update line strip and line segment in Constant Buffer of Hull Shader 
	CBUFFER_HULL_SHADER constantBuffer_HullShader;
	ZeroMemory((void *)&constantBuffer_HullShader, sizeof(CBUFFER_HULL_SHADER));

	constantBuffer_HullShader.Hull_Constant_Function_Params = XMVectorSet(1.0, uiNumberOfLineSegment, 0.0f, 0.0f);

	// smiliar to glUniformMatrix4fv() in OpenGL
	gpID3D11DeviceContext->UpdateSubresource(
		gpID3D11Buffer_ConstantBuffer_HullShader, 0, NULL, &constantBuffer_HullShader, 0, 0);

	// display changing number of line segment in windows caption bar
	TCHAR str[255];
	wsprintf(str, TEXT("Siddhant Ganesh Kshirsagar : [numberOfLineSegment = %d]"), uiNumberOfLineSegment);
	...
	// smiliar to glUniformMatrix4fv() in OpenGL
	gpID3D11DeviceContext->UpdateSubresource(
		gpID3D11Buffer_ConstantBuffer_PixelShader, 0, NULL, &constantBuffer_PixelShader, 0, 0);

	// set position buffer into pipeline here
	UINT stride = sizeof(float) * 2; // similar to glVertexAttribPointer() 3rd parameter
