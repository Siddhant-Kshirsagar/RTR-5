...
ID3D11VertexShader *gpID3D11VertexShader_Cube = NULL;
ID3D11PixelShader *gpID3D11PixelShader_Cube = NULL;
ID3D11InputLayout *gpID3D11InputLayout_Cube = NULL;

ID3D11Buffer *gpID3D11Buffer_PositionBuffer_Cube = NULL;
ID3D11Buffer *gpID3D11Buffer_TexcoordBuffer_Cube = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_Cube = NULL;
...
XMMATRIX perspectiveProjectionMatrix_Cube;

float cAngle = 0.0f;

// for sphere
ID3D11VertexShader *gpID3D11VertexShader_PV_Sphere = NULL;
ID3D11PixelShader *gpID3D11PixelShader_PV_Sphere = NULL;

ID3D11VertexShader *gpID3D11VertexShader_PF_Sphere = NULL;
ID3D11PixelShader *gpID3D11PixelShader_PF_Sphere = NULL;

ID3D11InputLayout *gpID3D11InputLayout_Sphere = NULL;
ID3D11Buffer *gpID3D11Buffer_PositionBuffer_Sphere = NULL;
ID3D11Buffer *gpID3D11Buffer_NormalBuffer_Sphere = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_Sphere = NULL;
ID3D11Buffer *gpID3D11Buffer_IndexBuffer_Sphere = NULL;


struct CBUFFER_SPHERE
{
	//XMMATRIX WorldViewProjectionMatrix; // similar to OpenGL model is equal to 'world' word
...
XMMATRIX perspectiveProjectionMatrix_Sphere;
...
char chooseShader = 'v'; // char 'v' for per vertex light / char 'f' per fragment light
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "initialize_Cube successfully\n\n");
		fclose(gpFILE);
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		{
			hr = resize_Cube(LOWORD(lParam), HIWORD(lParam));
			if (FAILED(hr))
			{
				gpFILE = fopen(gszLogFileName, "a+");
				fprintf(gpFILE, "resize_Cube() failed\n\n");
				fclose(gpFILE);
				return(hr);
			}
			else
			{
				gpFILE = fopen(gszLogFileName, "a+");
				fprintf(gpFILE, "resize_Cube() successfully\n\n");
				fclose(gpFILE);
			}
		}
		break;
...
{
	//function declarations
	HRESULT resize_Cube(int width, int height);
	...
	//code
	// 1: initialize_Cube swapchain descriptor structure
	...
	gpID3D11DeviceContext->VSSetShader(gpID3D11VertexShader_Cube, NULL, 0);
	...
	gpID3D11DeviceContext->PSSetShader(gpID3D11PixelShader_Cube, NULL, 0);
	...
	// initialize_Cube input element structure(input layout)
	...
	hr = gpID3D11Device->CreateInputLayout(d3dInputElementDesc, _ARRAYSIZE(d3dInputElementDesc), pID3DBlob_VertexShaderSourceCode->GetBufferPointer(), pID3DBlob_VertexShaderSourceCode->GetBufferSize(), &gpID3D11InputLayout_Cube);
	...
	gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayout_Cube);
	...
	// we will set above buffer into pipeline not here but in display_Cube because of our buffer is dynamic
	// 
	...
	d3d11BufferDesc.ByteWidth = sizeof(CBUFFER_CUBE);
	d3d11BufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	// create constant buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_ConstantBuffer_Cube);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->VSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_Cube);
	...
	hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpID3D11SamplerState_Cube);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	perspectiveProjectionMatrix_Cube = XMMatrixIdentity();


	// warmup resize
	hr = resize_Cube(WIN_WIDTH, WIN_HEIGHT);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "resize_Cube() failed\n\n");
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "resize_Cube() succeeded\n\n");
		fclose(gpFILE);
	}
	return(hr);
}
...
HRESULT initialize_Sphere(void)
{
	...
	// Per Vertex
	{
		...
		// using above structure create input layout
		hr = gpID3D11Device->CreateInputLayout(d3dInputElementDesc, _ARRAYSIZE(d3dInputElementDesc), pID3DBlob_VertexShaderSourceCode->GetBufferPointer(), pID3DBlob_VertexShaderSourceCode->GetBufferSize(), &gpID3D11InputLayout_Sphere);
		...
		// set above input layout in pipe
		gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayout_Sphere);
	...
	// Per Pixel
	{
		...
		const char *vertexShaderSourceCode_PF =
			...
			"for(int i = 0; i<3; i++)" \
			"{" \
			"output.lightDirection[i] = (lightPosition[i] - eyeCoordinates);" \
			"}" \
		...
		const char *pixelShaderSourceCode_PF =
			...
			"float3 ambientLight = lightAmbient[i] * materialAmbient;" \
			"float3 diffuseLight = lightDiffuse[i] * materialDiffuse * max(dot(normalizedLightDirection,normalizedTransfomedNormals),0.0);" \
			"float3 specularLight = lightSpecular[i] * materialSpecular * pow(max(dot(reflectionVector,normalizedViewerVector),0.0),materialShineness);" \
			"phongADSLight = phongADSLight + ambientLight + diffuseLight + specularLight;" \
			"}" \
			"}" \
			"else" \
			"{" \
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_PositionBuffer_Sphere);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for position vertex buffer\n\n");
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_NormalBuffer_Sphere);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_IndexBuffer_Sphere);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	// create constant buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_ConstantBuffer_Sphere);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for constant buffer\n\n");
	...
	// set above buffer into pipeline
	gpID3D11DeviceContext->VSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_Sphere);
	// set above buffer into pipeline
	gpID3D11DeviceContext->PSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_Sphere);
...
HRESULT resize_Cube(int width, int height)
{
	...
	// initialize_Cube and set the view // similar to openGL viewport
	...
	// initialize_Cube perspective projection

	perspectiveProjectionMatrix_Cube = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0f), (float)width / (float)height, 0.1f, 100.0f);
...
void display_Cube(void)
{
	...
	XMMATRIX wvpMatrix = worldMatrix * viewMatrix * perspectiveProjectionMatrix_Cube;

	CBUFFER_CUBE constantBuffer;
	ZeroMemory((void *)&constantBuffer, sizeof(CBUFFER_CUBE));
	constantBuffer.WorldViewProjectionMatrix = wvpMatrix;

	// smiliar to glUniformMatrix4fv() in OpenGL
	gpID3D11DeviceContext->UpdateSubresource(
		gpID3D11Buffer_ConstantBuffer_Cube, 0, NULL,&constantBuffer, 0, 0);
...
void uninitialize_Cube(void)
{
	...
	//code
	if (gpID3D11Buffer_ConstantBuffer_Cube)
	{
		gpID3D11Buffer_ConstantBuffer_Cube->Release();
		gpID3D11Buffer_ConstantBuffer_Cube = NULL;
	}
	...
	if (gpID3D11InputLayout_Cube)
	{
		gpID3D11InputLayout_Cube->Release();
		gpID3D11InputLayout_Cube = NULL;
	}
	if (gpID3D11PixelShader_Cube)
	{
		gpID3D11PixelShader_Cube->Release();
		gpID3D11PixelShader_Cube = NULL;
	}
	if (gpID3D11VertexShader_Cube)
	{
		gpID3D11VertexShader_Cube->Release();
		gpID3D11VertexShader_Cube = NULL;
	}
	if (gpID3D11SamplerState_Cube)
	{
		gpID3D11SamplerState_Cube->Release();
		gpID3D11SamplerState_Cube = NULL;
	}
