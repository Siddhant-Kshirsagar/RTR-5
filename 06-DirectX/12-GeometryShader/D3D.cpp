...
HRESULT initialize(void)
{
	...
	// In DirectX "Geometry SHader must not retain anything it should write it's output into the stream" This is rule : **** Refer to Pipeline diagram ****
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "Geometry shader compile successfully\n\n");
		fclose(gpFILE);
	}


	// create geometry shader from above code
	hr = gpID3D11Device->CreateGeometryShader(
		pID3DBlob_GeometryShaderSourceCode->GetBufferPointer(),
