...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_PositionBuffer_Triangle);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_PositionBuffer_Triangle,
		0,
	...
	// create vertex buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_ColorBuffer_Triangle);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for vertex buffer\n\n");
	...
	// create vertex buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_PositionBuffer_Rectangle);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() failed for vertex buffer\n\n");
	...
	// now map our position buffer with above resource
	gpID3D11DeviceContext->Map(gpID3D11Buffer_PositionBuffer_Rectangle,
		0,
		D3D11_MAP_WRITE_DISCARD,
		0,
		&d3dMappedSubresource);
	...
	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC; // similar to openGL GL_DYNAMIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(rectangle_color);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	// create color buffer using above structure
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_ColorBuffer_Rectangle);
	if (FAILED(hr))
	{
		gpFILE = fopen(gszLogFileName, "a+");
	...
	{
		gpFILE = fopen(gszLogFileName, "a+");
		fprintf(gpFILE, "CreateBuffer() succeeded for color buffer\n\n");
		fclose(gpFILE);
	}
	...
	// now copy actual data
	memcpy(d3dMappedSubresource.pData, rectangle_color, sizeof(rectangle_color));
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationX(-XMConvertToRadians(rAngle));
	translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
