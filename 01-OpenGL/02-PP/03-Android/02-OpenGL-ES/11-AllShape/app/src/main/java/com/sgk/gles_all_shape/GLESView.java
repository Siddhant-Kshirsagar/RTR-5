package com.sgk.gles_all_shape;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnDoubleTapListener,OnGestureListener
{
    ...
    private void initialize(GL10 gl)
    {
        ...
        float[] circle_position = new float[1083] ;
		int i = 0;

		for (int iAngle = 0; iAngle < 360; iAngle = iAngle + 1)
		{
			float x = 0.0f + 1.0f * (float) Math.cos((iAngle * Math.PI) / 180.0f);
			float y = 0.0f + 1.0f * (float)Math.sin((iAngle * Math.PI) / 180.0f);
			float z = 0.0f;
        ...
        positionBuffer.put(line_position);
        positionBuffer.position(0);

        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER,line_position.length * 4,positionBuffer,GLES32.GL_STATIC_DRAW);
        ...
        // vao(vertex array object) triangle
        ...
        // vao(vertex array object) square
        ...
        // prepare triangle vertices array for glBufferData
        byteBuffer = ByteBuffer.allocateDirect(square_position.length * 4);
        ...
        // vao(vertex array object) circle
        ...
        positionBuffer.put(circle_position);
        positionBuffer.position(0);

        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER,circle_position.length * 4,positionBuffer,GLES32.GL_STATIC_DRAW);
    ...
    void drawLine()
    {
        ...
        for(float xAxis = 0.05f; xAxis <= 2.0f; xAxis = xAxis + 0.05f)
        {
            ...
            Matrix.translateM(translationMatrix,0,xAxis,0.0f,-5.0f);
    
            Matrix.multiplyMM(modelViewMatrix,0,modelViewMatrix,0,translationMatrix,0);

            Matrix.multiplyMM(modelViewProjectionMatrix,0,perspectiveProjectionMatrix,0,modelViewMatrix,0);
            
            GLES32.glUniform4f(colorUniform,0.0f,0.0f,1.0f,1.0f);
        ...
        for(float yAxis = 0.05f; yAxis <= 2.0f; yAxis = yAxis + 0.05f)
        {
            ...
            Matrix.translateM(translationMatrix,0,0.0f,yAxis,-5.0f);
        ...
        // negative y axis (Horizontal Line)
        for(float yAxis = -0.05f; yAxis >= -2.0f; yAxis = yAxis - 0.05f)
        {
            lineCount = lineCount + 1;
            if (lineCount % 5 == 0)
        ...
        // center horizontal line
        {
            ...
            GLES32.glUniform4f(colorUniform,1.0f,0.0f,0.0f,1.0f);
        ...
        // center vertical line
        {
            ...
            GLES32.glUniform4f(colorUniform,0.0f,1.0f,0.0f,1.0f);
    ...
    void drawTriangle()
    {
        ...
        GLES32.glUniform4f(colorUniform,1.0f,1.0f,0.0f,1.0f);
