...
-(int)initialize
{
    ...
    glGenBuffers(1, &vbo_texCoordSquare);

    // step 20 : bind with VBO( Vertex Buffer Object) for position
    glBindBuffer(GL_ARRAY_BUFFER, vbo_texCoordSquare);
    ...
    texture_smiley = [self loadGLTextureFromImageFile: @"marble" : @"bmp"];
    if(texture_smiley == 0)
    {
        printf("Load of smiley texture is failed.\n");
        [self uninitialize];
        [self release];
        exit(0);
    }
...
-(void)display
{
   //code
    ...
    // step 2 : bind with vao_square(vertex array object)
    glBindVertexArray(vao_square);
...
-(void)uninitialize
{
    ...
    if (vbo_texCoordSquare)
    {
        glDeleteBuffers(1, &vbo_texCoordSquare);
        vbo_texCoordSquare = 0;
    }
