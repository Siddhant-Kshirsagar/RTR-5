...
{
	//function declarations
	// this function draw rectangle using multiple Quads
	//1st param top x-axis of left most coordinate of Quad(data type float)
	//2nd param top y-axis of left most coordinate of Quad(data type float)
	//3rd param right most coordinate for rectangle pattern (length of pattern)(data type float)
	void RectUsingQuad(float,float,float);
	...
	// draw multiple row of rectangle pattern 
	for(float fRow = 0.8f;fRow>-0.8f; fRow= fRow - 0.4f)
	{
		RectUsingQuad(-0.8f,fRow,0.8f);
	}
...
void RectUsingQuad(float xAxis,float yAxis,float length)
{
	...
	for(float fxAxis = (xAxis+0.2f); (fxAxis+0.2f)<length; fxAxis = fxAxis+0.2f)
	{
		//because of GL_QUAD_STRIP following two vertices are the 3rd and 4th vertices of next quad
