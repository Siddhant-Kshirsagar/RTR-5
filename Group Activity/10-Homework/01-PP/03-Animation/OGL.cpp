...
GLfloat pTranslate = 0.0f;
GLfloat cTranslate = 0.0f;

BOOL gbTranslate = FALSE;
...
void display(void)
{
	...
	translationMatrix = vmath::translate(pTranslate, 0.0f, -6.0f);
	...
	translationMatrix = vmath::translate(cTranslate, 0.0f, -6.0f);
...
void update(void)
{
	...
	if (gbTranslate == FALSE)
	{
		if (cTranslate >= -3.0f)
		{
			cTranslate = cTranslate - 0.01f;
		}

		if (pTranslate <= 3.0f)
		{
			pTranslate = pTranslate + 0.01f;
		}
		else
		{
			gbTranslate = TRUE;
