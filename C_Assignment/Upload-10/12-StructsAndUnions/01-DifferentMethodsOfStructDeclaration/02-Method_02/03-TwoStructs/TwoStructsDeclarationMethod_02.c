...
struct MyPoint SGK_point; //declaring a single variable of type 'struct MyPoint' globally...

struct MyPointProperties SGK_point_properties; // declaring a single variable of type 'struct MyPointProperties' globally...
...
int main(void)
{
	...
	else //Atleast One Of The Two Values (either 'X' or 'Y' or Both) is a non-zero value...
	{
		if (SGK_point.SGK_x == 0) // If 'X' Is Zero...Obviously 'Y' Is The Non-Zero Value
		{
			...
			SGK_point_properties.SGK_quadrant = 0; //A Point Lying On Any Of The Co-ordinate Is Not A Part Of Any Quadrant...
		...
		{
			SGK_point_properties.SGK_axis_location[0] = '\0'; // A Point Lying In Any Of The 4 Qudrants Cannot Be Lying On Any Of The Co-ordinate Axes
