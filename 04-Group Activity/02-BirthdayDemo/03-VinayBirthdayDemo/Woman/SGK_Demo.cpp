...
GLfloat gfWalkChangeFactor = 0.5f;
...
void updateScene(void)
{
	if (gbStartWalkAnimation)
	{
		if (gbWalkAnimation == FALSE)
		{
			...
			{
				gfRightShoulderAngle = gfRightShoulderAngle + gfWalkChangeFactor;

				if (gfRightElbowAngle <= 360.0f)
				{
					gfRightElbowAngle = gfRightElbowAngle + gfWalkChangeFactor;
				}
			}
			...
			{
				gfLeftShoulderAngle = gfLeftShoulderAngle - gfWalkChangeFactor;
				if (gfLeftElbowAngle >= 350.0f)
				{
					gfLeftElbowAngle = gfLeftElbowAngle - gfWalkChangeFactor;
				}
			}

			// for left leg
			if (gfLeftLegAngle >= 345.0f)
			{
				gfLeftLegAngle = gfLeftLegAngle - gfWalkChangeFactor;
				if (gfLeftKneeAngle >= 0.0f)
				{
					gfLeftKneeAngle = gfLeftKneeAngle - gfWalkChangeFactor;
				}
			}
			...
			{
				gfRightLegAngle = gfRightLegAngle + gfWalkChangeFactor;

				if (gfRightKneeAngle <= 40.0f)
				{
					gfRightKneeAngle = gfRightKneeAngle + gfWalkChangeFactor;
				}
			}
			else
			{
...
void drawTrunk(float xAxis, float yAxis, float zAxis)
{
	...
	glPushMatrix();
	{
		glTranslatef(xAxis, yAxis-1.4f, zAxis-0.15f);
		glPushMatrix();
		{
			glRotatef(-90.0f, 1.0f, 0.0f, 0.0f);
			...
			{
				//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
				//glTranslatef(xAxis, yAxis + 0.3f, zAxis - 0.15f);
				glPushMatrix(); // for cover base waist
				{
					//glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
					glTranslatef(0.0f, 0.0f, -0.2f); // after rotate x = x , y = z , z = y
					glScalef(1.2f, 0.6f, 1.0f);
					glRotatef(45.0f, 0.0f, 0.0f, 1.0f);
					gluCylinder(quadric, 0.4f, 0.4f, 0.3f, 20, 1);
				}
				glPopMatrix();

				glPushMatrix();// for below waist
				{
			...
			glPushMatrix(); // trunk base sphere
			{
			...
			glPushMatrix(); // trunk start
			{	
				glScalef(1.2f, 0.6f, 1.0f);
				gluCylinder(quadric, 0.4f, 0.32f, 0.5f, 20, 1); 
			}
			glPopMatrix();

			//glPushMatrix(); // middle abdomen
			//{
			//	glTranslatef(0.0f, 0.0f, zAxis + 0.5f);
			//	glScalef(1.2f, 0.6f, 0.4f);
			...
			glPushMatrix(); // middle abdomen 
			{
				glTranslatef(0.0f, 0.0f, zAxis + 0.5f);
				glScalef(1.2f, 0.6f, 1.0f);
				gluCylinder(quadric, 0.32f, 0.45f, 0.9f, 20, 1);
			}
			glPopMatrix();
...
void drawLegs(float xAxis, float yAxis, float zAxis)
{
	...
	{
		glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
		glTranslatef(0.0f, -0.15f, 1.7f); // after rotate x = x , y = z , z = y
		glScalef(1.2f, 0.6f, 1.0f);
		glRotatef(45.0f, 0.0f, 0.0f, 1.0f);
		gluCylinder(quadric, 0.4f, 0.5f, 0.4f, 20, 1);

		glTranslatef(0.0f, 0.0f, 0.4f); // after rotate x = x , y = z , z = y
		gluCylinder(quadric, 0.5f, 0.65f, 0.9f, 20, 1);

		glTranslatef(0.0f, 0.0f, 0.9f); // after rotate x = x , y = z , z = y
		gluCylinder(quadric, 0.65f, 0.7f, 1.0f, 20, 1);
	}
	glPopMatrix();
