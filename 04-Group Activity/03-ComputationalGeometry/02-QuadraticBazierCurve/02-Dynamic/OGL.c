...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
	{
		...
		case 'a':
			if (selected_point == 1)
				objX_1 -= scaleIncrement;
			else if (selected_point == 2)
				objX_2 -= scaleIncrement;
			else if (selected_point == 3)
				objX_3 -= scaleIncrement;
			break;
		case 'D':
		case 'd':
			if (selected_point == 1)
		...
		case 'e':
			if (selected_point == 1)
				objY_1 += scaleIncrement;
			else if (selected_point == 2)
				objY_2 += scaleIncrement;
			else if (selected_point == 3)
				objY_3 += scaleIncrement;
			break;
		case 'Q':
		case 'q':
			if (selected_point == 1)
		...
		case 'w':
			if (selected_point == 1)
				objZ_1 -= scaleIncrement;
			else if (selected_point == 2)
				objZ_2 -= scaleIncrement;
			else if (selected_point == 3)
				objZ_3 -= scaleIncrement;
			break;
		case 'S':
		case 's':
			if (selected_point == 1)
...
void display(void)
{
	...
	// Parametric Equation of Quadratic Bazier Curve B(t) = (1-t)^2 * p0 + 2 * (1-t)*t * p1 + t^2 *p2
	// t is always in between 0 to 1
	...
	for (float t = 0.0f; t <= 1.0f; t = t + 0.001f)
	{
		...
		float x = pow((1 - t), 2) * objX_1 + 2 * (1 - t) * t * objX_2 + pow(t, 2) * objX_3;
		float y = pow((1 - t), 2) * objY_1 + 2 * (1 - t) * t * objY_2 + pow(t, 2) * objY_3;
		float z = pow((1 - t), 2) * objZ_1 + 2 * (1 - t) * t * objZ_2 + pow(t, 2) * objZ_3;
	...
	glVertex3f(objX_1, objY_1, objZ_1);
	...
	glVertex3f(objX_2, objY_2, objZ_2);
	...
	glVertex3f(objX_3, objY_3, objZ_3);
